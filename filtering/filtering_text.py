...
def get_top_k_snippets_wiki(file_name, caption):
    snippets = []
    with open(
        f"./data/retrieved/{file_name}/text_data/wikipedia_search.json", "r"
    ) as f:
        data = json.load(f)
    ...
    for index in indices:
        ...
        try:
            url = (
                ...
                + data["query"]["search"][index]["title"]
                + "&format=json"
            )
            response = requests.get(url)
            page = json.loads(response.text)
            cleaned_text[index] = {
                "pagetext": remove_html_tags(page["parse"]["text"]["*"]),
                "title": data["query"]["search"][index]["title"],
                "timestamp": data["query"]["search"][index]["timestamp"],
            }
            done += 1
        except:
            continue
        ...
        try:
            ...
            if "pagemap" in data["items"][index].keys():
                if "metatags" in data["items"][index]["pagemap"].keys():
                    timestamp = get_timestamp_metadata_google(
                        data["items"][index]["pagemap"]["metatags"][0]
                    )
            cleaned_text[index] = {
                "pagetext": text,
    ...
    for index in indices:
        ...
        try:
            ...
            cleaned_text[index] = {
                "pagetext": text,
                "link": link,
                "title": data["webPages"]["value"][index]["name"],
                "timestamp": None,
            }
            done += 1
        except:
...
def get_inverse_google(file_name):
    with open(
        f"./data/retrieved/{file_name}/text_data/inverse_google_data.json", "r"
    ) as f:
        data = json.load(f)
    new_data = {}
...
def get_clean_data_new_inverse_bing(file_name):
    with open(
        f"./data/retrieved/{file_name}/text_data/inverse_bing_data.json", "r"
    ) as f:
        data = json.load(f)
    new_data = {}
    for idx, item in enumerate(data):
