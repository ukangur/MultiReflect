...
def get_text_scores(file_name, caption, image_path, client):
    text_rel = relevance.get_text_to_image_relevance_sample(
        file_name, image_path, client
    )
    ...
    with open(
        f"./data/ranking_score/{file_name}/text_data/authoritative.json", "w"
    ) as f:
    ...
    df.to_csv(
        f"./data/ranking_score/{file_name}/text_data/final_scores.csv", index=False
    )
...
def get_image_scores(file_name, caption, image_path, client):
    ...
    for idx in range(len(evidence)):
        ...
        try:
            ...
            metrics["usefulness"] = 0
        metrics["total"] = (
            metrics["relevance"] + metrics["support"] + metrics["usefulness"]
        )
        final_metrics.append(metrics)

    df = pd.DataFrame(final_metrics)
    df.to_csv(
        f"./data/ranking_score/{file_name}/image_data/final_scores.csv", index=False
    )
