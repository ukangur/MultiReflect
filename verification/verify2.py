...
def deepseekvl2_verification_prompt(image_path, caption, text_evidences, image_evidences):
    text_evidences = ""
    for i in range(len(text_evidences)):
...
def get_response_subs(file_name, image_path, caption, text_evidences, image_evidences, client):
    ...
    if config['client']=="llama":
        prompt = llava_cot_verification_prompt(image_path, caption, text_evidences, image_evidences)
        response = get_llava_cot_response(prompt,client)
    elif config['client']=="deepseek":
        prompt = deepseekvl2_verification_prompt(image_path, caption, text_evidences, image_evidences)
        response = get_deepseek_vl2_response(prompt,client)
    else:
        prompt = gpt4v_verification_prompt(image_path, caption, text_evidences, image_evidences)
